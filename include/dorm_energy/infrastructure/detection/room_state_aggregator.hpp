...
namespace dorm_energy::detection
{
    ...
    {
    public:
        std::optional<core::RoomState> update(const core::SensorReading &reading);

        const std::deque<core::RoomState> &getHistory(const std::string &deviceId) const;

        const std::unordered_map<std::string, core::RoomState> &getCurrentStates() const;

    private:
        std::unordered_map<std::string, core::RoomState> states_;
